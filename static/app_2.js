...
function updateCpuStat(value) {
  cpuStat.textContent = `${value}%`;
  cpuStat.style.color = value > 50 ? 'red' : '';
}

function updateRamStat(value) {
  ramStat.textContent = `${value}%`;
  ramStat.style.color = value > 50 ? 'red' : '';
}
...
function renderLogs(logs) {
  ...
  //logBox.scrollTop = 0;
  console.log(logBox.textContent);
  updateLogMeta();
}
...
async function loadState() {
  ...
  updateCpuStat(data.cpu_percent || 0);
  updateRamStat(data.ram_percent || 0);
...
function handleSocketMessage(msg) {
  if (msg.type === "state") {
    ...
    updateCpuStat(msg.cpu_percent || 0);
    updateRamStat(msg.ram_percent || 0);
